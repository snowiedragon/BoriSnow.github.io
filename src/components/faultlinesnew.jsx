        ...
        class Vector {
            ...
            drawFault(){
                for(let i = 0; i < originX.length; i++){ //scans each point
                    ...
                    let dy = 0;
                    // c.beginPath();
                    // c.arc(originX[i], originY[i], vectorEffectRadius, 0, Math.PI*2);
                    // c.stroke();
                    // c.closePath();
                    c.beginPath();
                    ...
                    while(originX[i] > 0 || originY[i] < 500){ //until the line reaches an edge
                        for(let u = 0; u < 2; u++){
                            let closeVector = [];
                            for (let m = 0; m < vectorArray.length; m++){ //checks for vectors within range
                                distance1 = Math.sqrt((originX[i]-vectorArray[m].x1)*(originX[i]-vectorArray[m].x1)+(originY[i]-vectorArray[m].y1)*(originY[i]-vectorArray[m].y1));
                                distance2 = Math.sqrt((originX[i]-vectorArray[m].x2)*(originX[i]-vectorArray[m].x2)+(originY[i]-vectorArray[m].y2)*(originY[i]-vectorArray[m].y2));
                                if(distance1 <= vectorEffectRadius || distance2 <= vectorEffectRadius){
                                    closeVector.push(m)
                                }
                            }
                            for (let n = 0; n < closeVector.length; n++){ //adds the influence of each vector
